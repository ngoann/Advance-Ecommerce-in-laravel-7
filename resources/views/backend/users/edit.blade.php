...
<div class="row mx-2">
  ...
  <div class="col-12 col-lg-6">
		<div class="card">
      <h5 class="card-header">Update Wallet</h5>
      <div class="card-body">
				<h6>User Point: <strong>{{$user->point}}</strong></h6>
				@if(Session::has('message'))
					<p class="alert alert-info">{{ Session::get('message') }}</p>
				@endif
        <form method="post" action="{{route('users.update-point',$user->id)}}">
          @csrf 
          <div class="form-group">
					...
					<div class="form-group">
						<input id="inputAmount" type="number" value="1" min="1" name="amount" placeholder="Enter amount" class="form-control">
						@error('amount')
							<span class="text-danger">{{$message}}</span>
						@enderror
					</div>
